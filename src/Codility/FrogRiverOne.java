...
//	each element of array A is an integer within the range [1..X].
	...
	public static int solution(int X, int[] A) {
		if (X > A.length) return -1; // don't check anything if there's not enough moves to place leaves
		if (X == 1 && A[0] == 1) return 0;

		int furthestAccessible = 0;
		int secondsNeeded = -1;

		boolean[] placedLeaves = new boolean[X+1];
		placedLeaves[0] = true; // the starting position of the frog

		for (int i = 0; i < A.length; i++){
			placedLeaves[A[i]] = true;
			int aux = updateFurthest(placedLeaves, furthestAccessible);
			if (aux > -1) furthestAccessible = aux;
			secondsNeeded++;
			if (i >= X) {
				if (furthestAccessible >= X) return secondsNeeded;
			}
		}
		return -1;
	}

	private static int updateFurthest (boolean[] placedLeaves, int furthestAccessible) {
		for (int i = furthestAccessible; i < placedLeaves.length; i++) {
			if (!placedLeaves[i]) return i-1;
			else if (i == placedLeaves.length-1 && placedLeaves[i]) return i;
		}
		return furthestAccessible;
	}
