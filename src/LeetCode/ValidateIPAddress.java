...
//	Explanation: This is neither a IPv4 address nor a IPv6 address.
	...
	private static String work (String IP){
		String invalid = "Neither";

		if (IP.contains(".")) { // IPv4

			if (IP.charAt(0) == '.' || IP.charAt(IP.length()-1) == '.') return invalid;

			String[] IPv4 = IP.split("\\.");

			if (IPv4.length != 4) return invalid;

			for (int i = 0; i < IPv4.length; i++) {

				try {
					if (i == 0 || i == 3) {
						if (IPv4[i].startsWith("0") || IPv4[i].startsWith("00") || IPv4[i].startsWith("-")) return invalid;
					} else {
						if (IPv4[i].startsWith("00") || IPv4[i].startsWith("-")) return invalid;
					}

					int num = Integer.parseInt(IPv4[i]);
			...
			if (IPv6.length != 8) return invalid;

			for (int i = 0; i < IPv6.length; i++) {

				if (IPv6[i].length() > 4 || IPv6[i].startsWith("-")) return invalid;

				try {
					long hex = Long.decode("0x" + IPv6[i]);
					if (hex < 0 || hex > 65535) return invalid; }
	...
	public static String solution(String IP){
		...
		String solution = work(IP);
		System.err.println(solution);
		return solution;
	}

	public static void main(String[] args) {
		String IP = "192.0.0.1"; // "1e1.4.5.6"; // "172.16.254.1"; // "256.256.256.256"; //
